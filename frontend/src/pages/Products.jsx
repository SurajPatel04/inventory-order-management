      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100/60">
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <label className="flex items-center space-x-2 text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-xl border border-gray-100 cursor-pointer">
            <input type="checkbox" checked={filters.low_stock} onChange={e => setFilters({...filters, low_stock: e.target.checked, skip: 0})} className="rounded border-gray-300 text-[#432DD7] focus:ring-[#432DD7]" />
            <span>Low Stock</span>
          </label>
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 w-full sm:w-32 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#432DD7] focus:ring-2 focus:ring-[#432DD7]/20 text-sm outline-none transition-all"
            />
          </div>
        </div>
        <motion.button 
      ...
      <Modal isOpen={isFormOpen} onClose={closeForm} title={editingId ? 'Update Product' : 'Add New Product'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input required type="text" placeholder="e.g. Wireless Mouse" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="block w-full rounded-xl border-gray-200 bg-white shadow-sm p-2.5 border focus:border-[#432DD7] focus:ring-2 focus:ring-[#432DD7]/20 transition-all outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">SKU</label>
            <input required type="text" placeholder="e.g. WM-01" value={form.sku} onChange={e => setForm({...form, sku: e.target.value})} className="block w-full rounded-xl border-gray-200 bg-white shadow-sm p-2.5 border focus:border-[#432DD7] focus:ring-2 focus:ring-[#432DD7]/20 transition-all outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Price ($)</label>
              <input required type="number" placeholder="0.00" step="0.01" min="0" value={form.price} onChange={e => setForm({...form, price: e.target.value})} className="block w-full rounded-xl border-gray-200 bg-white shadow-sm p-2.5 border focus:border-[#432DD7] focus:ring-2 focus:ring-[#432DD7]/20 transition-all outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Stock Quantity</label>
              <input required type="number" placeholder="0" min="0" value={form.quantity} onChange={e => setForm({...form, quantity: e.target.value})} className="block w-full rounded-xl border-gray-200 bg-white shadow-sm p-2.5 border focus:border-[#432DD7] focus:ring-2 focus:ring-[#432DD7]/20 transition-all outline-none" />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-6">
            ...
            <button type="submit" className={`flex items-center py-2.5 px-6 rounded-xl shadow-sm text-sm font-semibold text-white transition-all outline-none ${editingId ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/20' : 'bg-[#432DD7] hover:bg-[#3422AD] shadow-[#432DD7]/20'}`}>
